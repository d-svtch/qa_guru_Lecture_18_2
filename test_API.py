...
from api_tools.api_tools import product_add
...
def test_add_product_to_cart():
    ...
    with step("Get API cookie"):
        auth_cookie = requests.get(API_URL).cookies.get("Nop.customer")
    ...
    with step("Check cart"):
        check_cart(auth_cookie, product_1["product_name"], product_1["quantity"])
...
def test_add_various_products():
    ...
    with step("Check cart"):
        for product in products:
            check_cart(auth_cookie, product["product_name"], product["quantity"])
